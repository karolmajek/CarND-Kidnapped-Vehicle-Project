...
int main()
{
    ...
    {
        cout << "Error: Could not open position/control measurement file"
                << endl;
        return -1;
    }
    ...
    for (int i = 0; i < num_time_steps; ++i)
    {
        ...
        if (!pf.initialized())
        {
            ...
            pf.init(gt[i].x + n_x, gt[i].y + n_y, gt[i].theta + n_theta,
                    sigma_pos);
        }
        else
        {
        ...
        cout << "Cumulative mean weighted error: x " << cum_mean_error[0]
                << " y " << cum_mean_error[1] << " yaw " << cum_mean_error[2]
                << endl;
    ...
    else
    {
        cout << "Your runtime " << runtime
                << " is larger than the maximum allowable runtime, "
                << max_runtime << endl;
        return -1;
    }
