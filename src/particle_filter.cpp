        ...
        double velocity, double yaw_rate)
{
    ...
    for(int i=0;i<particles.size();++i)
    {
        ...
        {
          particles[i].x += velocity * (sin(particles[i].theta + yaw) - sin(particles[i].theta)) / yaw_rate + dist_x(generator);
          particles[i].y += velocity * (cos(particles[i].theta) - cos(particles[i].theta + yaw)) / yaw_rate + dist_y(generator);
        ...
        std::vector<LandmarkObs> observations, Map map_landmarks)
{
    ...
    for (std::vector<Particle>::iterator it = particles.begin() ; it != particles.end(); ++it)
    {
        ...
        for(int o=0;o<observations.size();++o)
        {
            ...
            // Bivariate Gaussian
            double num_a = (dist_m-dist_o) * (dist_m-dist_o) / (2.0 * std_landmark[0] * std_landmark[0]);
            double num_b = (ang_m-ang_o) * (ang_m-ang_o) / (2.0 * std_landmark[1] * std_landmark[1]);
...
/*
    ...
    std::uniform_real_distribution<> uniform_double(0.0, 2.0 * maxw);
    ...
    for(int i=0;i<particles.size();++i)
    {
        ...
        new_particles[new_particles.size()-1].weight=1;
        weights[i]=1;
    }
